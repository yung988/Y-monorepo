...
async function getRandomProducts(
  ...
  try {
    const allProducts: Product[] = await (await import("@/lib/admin-api")).adminApi.getProducts();

    // Filtrujeme aktivní produkty a vylučujeme aktuální produkt
    const availableProducts = allProducts.filter(
      (product) => product.status === "active" && product.id !== currentProductId,
    );
    ...
    return shuffled.slice(0, limit).map((p) => ({
      ...
      status: p.status,
      image_url:
        p.product_images?.find((img) => img.is_main_image)?.url || p.product_images?.[0]?.url,
    }));
  } catch (error) {
