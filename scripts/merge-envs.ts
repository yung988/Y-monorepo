...
function main() {
  ...
  const backEnvPath = resolve(repoRoot, ".env back");

  if (!existsSync(frontEnvPath) && !existsSync(backEnvPath)) {
    console.error("No source env files found (.env front, .env back). Nothing to merge.");
    process.exit(1);
  }
  ...
  // Prepare frontend .env containing only NEXT_PUBLIC_* variables.
  const combinedPublic: Record<string, string> = {};
  ...
  // Prepare backend .env: take backVars base, and ensure public vars are present (prefer frontend if provided)
  const combinedBack: Record<string, string> = { ...backVars };
  ...
  // Write files
  const feOut = resolve(feDir, ".env");
  const beOut = resolve(beDir, ".env");
